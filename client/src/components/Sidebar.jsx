...
const Sidebar = () => {
    ...
    const {getUsers, users, selectedUser, setSelectedUser, unseenMessages, setUnseemMessages } = useContext(ChatContext);
  ...
  return (
    <div className={`bg-[#818582]/10 h-full p-5 rounded-r-xl overflow-y-scroll text-white ${selectedUser ? "max-md:hidden" : ''}`}>
        ...
        <div className='bg-[#282142] rounded-full flex items-center gap-2 py-3 px-4 mt-5'>
            ...
            <input onChange={(e)=>setInput(e.target.value)} type="text" className='bg-transparent border-none outline-none text-white text-xs placeholder-[#c8c8c8] flex-1' placeholder='Search User...' name="" id="" />
        </div> 
       ...
       <div className='flex flex-col'>
           {filteredUsers.map((user,index) => (
             <div onClick={()=> {setSelectedUser(user); setUnseemMessages(prev=>
             ({...prev,[user._id]:0})
             )}} 
